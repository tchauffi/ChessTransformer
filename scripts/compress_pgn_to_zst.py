...
def main():
    parser = argparse.ArgumentParser(description="Convert PGN files to PGN.ZST (Zstandard compressed) format")
    parser.add_argument("input_folder", type=str, help="Folder containing PGN files to compress")
    parser.add_argument(
        "output_folder",
        type=str,
        nargs="?",
    ...
    parser.add_argument("--level", type=int, default=3, help="Compression level (1-22, default: 3)")
    parser.add_argument("--pattern", type=str, default="*.pgn", help="File pattern to match (default: *.pgn)")
    parser.add_argument("--merge", action="store_true", help="Merge all PGN files into a single compressed file")
    parser.add_argument(
        "--output-name",
        type=str,
