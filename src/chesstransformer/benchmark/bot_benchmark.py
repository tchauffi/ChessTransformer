...
class BotBenchmark:
    ...
    def create_bot(
        self, bot_type: str, checkpoint_path: Optional[str] = None, bot_name: Optional[str] = None, **kwargs
    ):
        """
    ...
    def run_benchmark(
        self, bot1, bot2, num_games: int, alternate_colors: bool = True, verbose: bool = False
        ...
        for game_id in pbar:
            ...
            pbar.set_postfix({bot1._benchmark_id: bot1_wins, bot2._benchmark_id: bot2_wins})
    ...
    def save_results(self, output_path: str):
        ...
        output = {
            "config": {"max_moves": self.max_moves, "time_limit": self.time_limit, "num_games": len(self.game_results)},
            "games": [r.to_dict() for r in self.game_results],
            "stats": {
                bot_name: self._calculate_stats(bot_name).to_dict()
                for bot_name in set([r.white_bot for r in self.game_results] + [r.black_bot for r in self.game_results])
            },
        }
...
def main():
    ...
    parser.add_argument("--bot1-checkpoint", type=str, default=None, help="Checkpoint path for bot1 (if applicable)")
    parser.add_argument(
        "--bot1-name", type=str, default=None, help="Custom name for bot1 (useful when comparing same bot types)"
    )
    parser.add_argument(
        "--bot2-type",
        type=str,
    ...
    parser.add_argument("--bot2-checkpoint", type=str, default=None, help="Checkpoint path for bot2 (if applicable)")
    parser.add_argument(
        "--bot2-name", type=str, default=None, help="Custom name for bot2 (useful when comparing same bot types)"
    )
    parser.add_argument("--num-games", type=int, default=100, help="Number of games to play")
    parser.add_argument("--max-moves", type=int, default=500, help="Maximum moves per game")
    parser.add_argument("--time-limit", type=float, default=None, help="Time limit per move in seconds")
    parser.add_argument("--output", type=str, default="benchmark_results.json", help="Output file for results")
    parser.add_argument("--no-alternate", action="store_true", help="Don't alternate colors between games")
    ...
    bot1_stats, bot2_stats = benchmark.run_benchmark(
        bot1, bot2, num_games=args.num_games, alternate_colors=not args.no_alternate, verbose=args.verbose
    )
